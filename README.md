...
Чтобы воспроизвести потерю детекции при смазывании кадра, симулятор измеряет
угловую скорость направления на мяч относительно камеры — то есть скорость его
перемещения по полю зрения. Для каждого тренировочного эпизода порог выбирается
из диапазона `0.6…0.85 рад/с`; при превышении порога цель считается невидимой.
После замедления относительного движения детекция остаётся отключённой ещё
`0.2 с`. Параметры задаются в секции `SimulatedYoloCamera` файла
...
| ROS TCP | автообнаружение робота `158`, порт `10001` |
| Камера | `http://<discovered-robot-address>:10002/frame.jpg` |
| YOLO → Unity | UDP `5005` |
...
Третье continuous-действие остаётся абсолютной нормализованной целью камеры.
