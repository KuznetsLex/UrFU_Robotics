...
угловую скорость камеры относительно мира. Для каждого тренировочного эпизода
порог выбирается из диапазона `0.6…0.85 рад/с`; при превышении порога цель
считается невидимой. После замедления камеры детекция остаётся отключённой ещё
